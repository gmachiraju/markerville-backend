...
def add_adj_candidate_BC(session, BiomarkerCondition, candidates):
    with session.no_autoflush:
        ...
        for c in candidates:
            ...
            while(x >= 0 and temp_arr_poses[x] == 'jj'):
                cared_words.append(temp_arr_words[x])
                x -= 1
            to_append_string = ""
            ...
            if(len(to_append_string) > 0):
                ...
                ConditionSpan = Span(sentence=c.condition.sentence, char_start=(
                    c.condition.char_start - len(to_append_string)), char_end=c.condition.char_end)
                NewCandidate = BiomarkerCondition(
                    biomarker=BiomarkerSpan, condition=ConditionSpan)
        ...
        print len(needsAdding), 'candidates need adding'
        for thing in needsAdding:
            if(not thing in candidates):
                candidates.append(thing)
                # session.add(BiomarkerCondition())
            else:
                print "ASDF"
